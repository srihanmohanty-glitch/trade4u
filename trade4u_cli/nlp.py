...
from trade4u_cli.data.stocks import ALL_STOCKS, SYMBOL_EXCHANGES
...
INTENTS = {
    "PRICE": {
        ...
        "patterns": [
            r"\bat\s+\$?(\d+)",
            r"\bwhat'?s?\s+(\w+)\s+(at|trading|worth)",
            r"\bhow much is\s+(\w+)",
            r"how'?s?\s+(\w+)\s+(doing|looking|trading)",
            r"(?:what|how)\s+(?:is|are)\s+(\w+)\s+doing",
        ],
    },
    "PORTFOLIO": {
        "keywords": [
        ...
        "patterns": [
            r"show\s+(?:my\s+)?(?:portfolio|holdings|positions)",
            r"what\s+(?:do|i)\s+own",
            r"list\s+(?:my\s+)?(?:holdings|positions)",
        ],
    },
    "ADD_HOLDING": {
        "keywords": [
        ...
        "patterns": [
            r"(?:bought|purchased|acquired)\s+(\d+)\s+(\w+)",
            r"(\d+)\s+shares?\s+(?:of\s+)?(\w+)",
            r"(?:bought|purchased)\s+(\w+)\s+(\d+)",
            r"i own (\d+)\s+(\w+)",
            r"add\s+(\w+)\s+(\d+)",
        ],
    },
    "REMOVE_HOLDING": {
        ...
        "patterns": [
            r"(?:sold|remove|liquidate)\s+(?:all\s+)?(\w+)",
            r"close\s+(?:my\s+)?(\w+)",
            r"sell\s+(?:all\s+)?(\w+)",
        ],
    },
    "ALERT": {
        ...
        "patterns": [
            r"alert\s+(?:me\s+)?(?:when|if)\s+(\w+)\s+(hits?|goes?|reaches?|drops?|falls?|above|below)\s+\$?(\d+)",
            r"notify\s+me\s+when\s+(\w+)\s+\$?(\d+)",
            r"watch\s+(\w+)\s+(?:at|when|if)\s*\$?(\d+)",
            r"(?:watch|monitor)\s+(\w+)",
            r"(?:let|keep)\s+me\s+know\s+when",
        ],
    },
    "NEWS": {
        "keywords": [
    ...
    "TOP_GAINERS": {
        ...
        "patterns": [
            r"(?:top|best)\s+(?:gaining|gainers|performers)",
            r"stocks?\s+(?:up|gaining)",
            r"(?:who'?s|which\s+are)\s+(?:winning|up|gaining)",
            r"best\s+(?:performing|trending)",
        ],
    },
    "TOP_LOSERS": {
        "keywords": [
        ...
        "patterns": [
            r"(?:top|worst)\s+(?:losing|losers|performers)",
            r"stocks?\s+(?:down|losing)",
            r"(?:who'?s|which\s+are)\s+(?:losing|down)",
        ],
    },
    "INDEX": {
        "keywords": [
    ...
    "CRYPTO": {
        ...
        "patterns": [
            r"(?:bitcoin|btc|ethereum|eth|crypto)\s+(?:price|at|worth)",
            r"(?:what|how)\s+(?:is|are)\s+(?:bitcoin|btc|eth)",
        ],
    },
    "HELP": {
        "keywords": [
    ...
    "ADD_CASH": {
        ...
        "patterns": [
            r"(?:add|deposit|top\s*up)\s+\$?(\d+)",
            r"(?:add|deposit)\s+(?:cash|money|funds)\s+\$?(\d+)",
            r"put\s+\$?(\d+)\s+(?:in|cash)",
        ],
    },
    "52WEEK": {
        ...
        "patterns": [
            ...
            r"(?:all\s+)?time\s+(high|low|high|low)",
            r"support\s+level",
    ...
    "EARNINGS": {
        ...
        "patterns": [
            r"(?:earnings|revenue)\s+(?:report|beat|miss|estimate)",
            r"quarter\s+(?:results|report)",
        ],
    },
    "PE_RATIO": {
        "keywords": [
    ...
    "RECOMMEND": {
        ...
        "patterns": [
            r"(?:which|what|give me|recommend|suggest)\s+.*(?:buy|pick|stock|invest)",
            r"best\s+(?:stock|pick|buy)",
            r"(?:hot|top|momentum)\s+stock",
            r"what\s+to\s+buy",
            r"investment\s+idea",
            r"(?:short|long|intraday)\s*term",
            r"(?:what|show|list)\s+stocks?\s+(?:can|to|available)",
            r"available\s+stocks",
        ],
    },
    "AI": {
        "keywords": [
        ...
        "patterns": [
            r"what\s+(do\s+you\s+think|do\s+you\s+say|about|your\s+opinion)",
            r"your\s+(thoughts?|opinion|view|advice)",
            r"(should|could)\s+you\s+(explain|analyze|tell)",
            r"why\s+(do\s+you|is|are|should)",
            r"(?:give\s+me\s+)?(?:your\s+)?(?:opinion|thoughts?|advice|insight)",
        ],
    },
}
...
def classify_intent(text):
    ...
    if re.search(
        r"\b(who\s+(are|is)\s+(you|this)|what\s+(are|is)\s+(you|this)|your\s+name)\b", text_lower
    ):
        return "WHO"

    if re.search(
        r"\b(what\s+(do\s+i\s+own|am\s+i\s+holding)|show\s+(my\s+)?(portfolio|holdings|positions))\b",
        text_lower,
    ):
        return "PORTFOLIO"
    ...
    if re.search(r"\b(watch|alert|notify|monitor|track)\s+\w+", text_lower) and re.search(
        r"\b\d+\b", text_lower
    ):
        return "ALERT"
    ...
    if re.search(
        r"\b(buy|pick|suggest|recommend|which|what to|instant)\s+.*(stock|pick|invest|trade)\b",
        text_lower,
    ):
        return "RECOMMEND"
    ...
    if re.search(
        r"\b(what\s+(is|was)\s+happening|latest\s+news|recent\s+news|headlines|market\s+news)\b",
        text_lower,
    ):
        return "NEWS"

    if re.search(
        r"\b(any|latest|market)\s+(market\s+)?(info|information|update|data)\b", text_lower
    ):
        return "NEWS"

    if re.search(
        r"\b(show|get|fetch|fetch|retrieve|give|give me)\s+.*(price|quote|value|info|data|details)\b",
        text_lower,
    ):
        return "PRICE"

    if re.search(
        r"\b(how|what)\s+(is|are|was|were)\s+(\w+)\s+(doing|looking|trading|worth|valued)\b",
        text_lower,
    ):
        return "PRICE"
    ...
    if re.search(
        r"\b(anything|everything)\s+(about|on)\s+(the\s+)?(market|stocks|stock|crypto)\b",
        text_lower,
    ):
        return "NEWS"
    ...
    if re.search(
        r"\b(best|top)\s+(performing|performers?|gaining|gainers?|stocks?|movers|level|tier)\b",
        text_lower,
    ):
        return "TOP_GAINERS"

    if re.search(
        r"\b(how|what)\s+(is|are|was|were)\s+(\w+)\s+(doing|looking|trading)\b", text_lower
    ):
        return "PRICE"

    text_lower_clean = (
        text_lower.replace("what is", "")
        .replace("how much", "")
        .replace("what's", "")
        .replace("how's", "")
    )
...
def extract_entities(text):
    ...
    stock_symbols = list(ALL_STOCKS.keys())
...
def parse_command(text):
    ...
    if intent == "ADD_HOLDING":
        ...
        match = re.search(
            r"(?:bought|purchased)\s+(\w+)\s+(?:at\s+)?\$?(\d+(?:\.\d+)?)", text_lower
        )
        if match:
