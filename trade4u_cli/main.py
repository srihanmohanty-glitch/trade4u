...
from trade4u_cli.data.stocks import ALL_STOCKS, STOCK_LISTS, get_yahoo_symbol, SYMBOL_EXCHANGES
...
Just ask naturally!"""
        ...
        if intent == "HOW_ARE_YOU":
            return (
                "I'm doing great! Ready to help you with the markets. What would you like to know?"
            )
        ...
        if intent == "EARNINGS":
            ...
            if symbol:
                return (
                    f"Earnings data for {symbol} - Check financial news for latest earnings report."
                )
        ...
        if intent == "PE_RATIO":
            ...
            if symbol:
                return (
                    f"P/E ratio for {symbol} - Check financial data for current valuation metrics."
                )
...
Or install Ollama to enable AI for any question!"""
    ...
    def handle_top_movers(self, movers_type):
        ...
        symbols = STOCK_LISTS.get(stock_list, STOCK_LISTS["default"]).get(
            exchange, STOCK_LISTS["default"]["NYSE"]
        )

        quotes = get_multiple_quotes(symbols)

        sorted_quotes = sorted(
            quotes.values(), key=lambda x: x["change_pct"], reverse=(movers_type == "gainers")
        )
        top = sorted_quotes[:5]
        ...
        for q in top:
            ...
            lines.append(
                f"{q['symbol']}: {sign}{q['change_pct']:.2f}% ({curr_symbol}{q['price']:,.2f})"
            )
    ...
    def handle_recommend(self, user_input):
        ...
        if "short" in text or "intraday" in text or "today" in text:
            ...
            sorted_quotes = sorted(
                valid_quotes, key=lambda x: x.get("market_cap") or x.get("price") or 0, reverse=True
            )[:5]
        ...
        lines = [
            title,
            f"*{exchange}/{stock_list} • {currency} • {settings.get('timezone', 'UTC')}*",
            "",
        ]
        for q in sorted_quotes:
            sign = "+" if q["change_pct"] >= 0 else ""
            lines.append(
                f"{q['symbol']}: {sign}{q['change_pct']:.2f}% today ({curr_symbol}{q['price']:,.2f})"
            )

        lines.append("")
        lines.append(
            "*Note: These are just suggestions based on market data. Do your own research!*"
        )
    ...
    def handle_price(self, symbol):
        ...
        if not quote:
            return (
                f"I couldn't find data for {symbol}. Try another symbol like AAPL, TSLA, or NVDA."
            )
...
52W Range: {range_str}{market_cap}"""
    ...
    def handle_portfolio(self):
        ...
        for h in holdings:
            ...
            sign = "+" if pnl >= 0 else ""
            lines.append(
                f"{emoji} {h['symbol']}: {h['shares']} shares | ${value:,.2f} ({sign}{pnl_pct:.1f}%)"
            )
    ...
    def handle_remove_holding(self, symbol):
        ...
        portfolio["holdings"] = [
            h for h in portfolio["holdings"] if h["symbol"].upper() != symbol.upper()
        ]
    ...
    while True:
        try:
            ...
            if user_input.lower().strip() == "-settings":
                ...
                console.print(
                    f"  📋 Stock List: [cyan]{settings.get('stock_list', 'default')}[/cyan]"
                )
                ...
                console.print(
                    f"  🔔 Alerts: [cyan]{'Enabled' if settings['alerts_enabled'] else 'Disabled'}[/cyan]\n"
                )
                ...
                if change.lower() == "y":
                    ...
                    choice = Prompt.ask(
                        "[bold]Enter number (or press Enter to skip):[/bold]", default=""
                    )
                    if choice.strip() == "1":
                        new_val = Prompt.ask(
                            "[bold]Enter exchange:[/bold]", default=settings["exchange"]
                        )
                        ...
                        for lst in STOCK_LISTS:
                            console.print(f"  • {lst}")
                        new_val = Prompt.ask(
                            "[bold]Enter stock list:[/bold]",
                            default=settings.get("stock_list", "default"),
                        )
                        settings["stock_list"] = new_val.lower()
                    elif choice.strip() == "3":
                        new_val = Prompt.ask(
                            "[bold]Enter timezone:[/bold]", default=settings["timezone"]
                        )
                        settings["timezone"] = new_val
                    elif choice.strip() == "4":
                        new_val = Prompt.ask(
                            "[bold]Enter currency:[/bold]", default=settings["currency"]
                        )
                        settings["currency"] = new_val.upper()
                    elif choice.strip() == "5":
                        new_val = Prompt.ask(
                            "[bold]Display (compact/detailed):[/bold]", default=settings["display"]
                        )
                        settings["display"] = new_val.lower()
                    elif choice.strip() == "6":
                        new_val = Prompt.ask(
                            "[bold]Alerts (on/off):[/bold]",
                            default="on" if settings["alerts_enabled"] else "off",
                        )
